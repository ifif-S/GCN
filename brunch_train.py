...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 31 12:50:38 2018
...
if opt.use_brunch:
    indices = get_indice_graph(
                    adj, train_mask, opt.batch_size, 1.0)

    brunch_adj = normed_adj[indices,:][:,indices]
    brunch_features = feature[indices,:][:,indices]
    output, hid_gcn, hid_gat = model_x(brunch_adj, brunch_adj)
...
def init_train():
    ...
    criterion = nn.CrossEntropyLoss()
    criterion_neg = nn.Sigmoid ()
    optimizer = torch.optim.Adam(model_gx.parameters(), lr=opt.graph_learning_rate)
    optimizer.zero_grad()

    for i in range(opt.init_iter_graph):
        gx, gy, gz = next(gen_graph(graph, features, opt.neg_samp))
        if opt.neg_samp > 0:
            l_gy, l_gx = model_gx(gx, gy, adj)
            gz=torch.tensor(gz)
            g_loss = - torch.log( criterion_neg( torch.sum(l_gx * l_gy, dim = 1) * gz )  ) .sum()
        else:
            l_gx = model_gx(gx, gy, adj)
